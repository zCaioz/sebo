...
/**
 * REST controller for managing {@link br.com.uem.sebo.domain.Item}.
 */
@RestController
    ...
    public ResponseEntity<Item> partialUpdateItem(
        ...
        Optional<Item> result = itemRepository
            ...
            .map(existingItem -> {
                ...
                if (item.getAno() != null) {
                    existingItem.setAno(item.getAno());
                }
                if (item.getGenero() != null) {
                    existingItem.setGenero(item.getGenero());
                }
                if (item.getAutorArtista() != null) {
                    existingItem.setAutorArtista(item.getAutorArtista());
                }
                if (item.getDisponibilidade() != null) {
                    existingItem.setDisponibilidade(item.getDisponibilidade());
                }
